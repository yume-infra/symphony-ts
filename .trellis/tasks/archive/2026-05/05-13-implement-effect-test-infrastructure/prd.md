# Implement Effect-first test infrastructure
...
- [x] `apps/cli` has a shared Effect test helper.
- [x] The helper can run a successful Effect and return its value in a Vitest test.
- [x] The helper surfaces Effect failures with useful Cause/error detail.
- [x] Test-support directories exist for fakes/fixtures without introducing runtime behavior.
- [x] Placeholder fake boundaries are documented or typed enough for future runtime tests to extend.
- [x] At least one real test covers current CLI logic.
- [x] `passWithNoTests` is removed or no longer required for the normal test path.
- [x] `pnpm --filter symphony-ts test` passes.
- [x] `pnpm verify` passes.
- [x] `pnpm smoke:bin` still passes.
- [x] No Symphony runtime implementation is added.
